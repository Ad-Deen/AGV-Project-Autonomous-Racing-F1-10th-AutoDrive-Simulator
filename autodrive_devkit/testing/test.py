...
output_png_path = '/home/autodrive_devkit/src/racing_agent/upsampled_racetrack_map_with_first_intersections.png'
...
def plot_upsampled_map_with_first_intersections(outer_line, inner_line, output_path):
    """Plot the upsampled outer, inner, circles, and the first intersection points."""
    ...
    first_intersection_points = []
    
    # Plot circles and calculate only the first intersection points for consecutive circles
    radius = 0.4
    for i in range(len(inner_x) - 1):
        ...
        # Plot circles
        circle1 = plt.Circle((x1, y1), radius, color='g', fill=False, lw=1.5)
        circle2 = plt.Circle((x2, y2), radius, color='g', fill=False, lw=1.5)
        ...
        if intersect:
            (ix1, iy1), _ = intersect  # Only take the first intersection point
            
            first_intersection_points.append((ix1, iy1))
            
            # Plot first intersection point
            plt.scatter([ix1], [iy1], c='k', s=50, zorder=5, label='First Intersection Point' if i == 0 else None)
    
    plt.xlabel('X Coordinate')
    plt.ylabel('Y Coordinate')
    plt.title('Upsampled Racetrack Map with First Intersection Points')
    plt.legend()
    plt.grid(True)
...
def main():
    ...
    # Plot the upsampled map with circles and first intersection points
    plot_upsampled_map_with_first_intersections(outer_line, inner_line, output_png_path)

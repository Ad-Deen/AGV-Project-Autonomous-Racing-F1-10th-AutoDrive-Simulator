...
def plot_racetracks(inner_x, inner_y, outer_x, outer_y, inner_curve_x, inner_curve_y, outer_curve_x, outer_curve_y):
    """Plot the inner and outer racelines with smooth curves."""
    plt.figure(figsize=(12, 8))

    # Plot the inner and outer racelines
    # plt.plot(outer_x, outer_y, 'b-', lw=2, label='Original Outer Boundary')
    # plt.plot(inner_x, inner_y, 'r-', lw=2, label='Original Inner Boundary')
    ...
    plt.title('Smooth Racetrack Map with Inner and Outer Boundaries')
    plt.legend()
    plt.grid(True)
    plt.show()

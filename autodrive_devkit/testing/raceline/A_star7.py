...
def plot_optimal_path(path, ax):
    """Plot the optimal path using a continuous line on the ax."""
    x_vals = [node[0] for node in path]  # Extract x coordinates from path
    y_vals = [node[1] for node in path]  # Extract y coordinates from path
    
    # Plot the optimal path with a blue line
    ax.plot(x_vals, y_vals, color='blue', linewidth=2, label='Optimal Path')

    # Mark the start and goal points
    ax.scatter(x_vals[0], y_vals[0], color='green', s=100, label='Start')
    ax.scatter(x_vals[-1], y_vals[-1], color='red', s=100, label='Goal')
...
def a_star_pathfinding(start_x, start_y, start_orientation):
    ...
    while open_list:
        ...
        for neighbor in neighbors:
            ...
            if (neighbor_x, neighbor_y) not in closed_list:
                ...
                # Update parent_map: make the current node the parent of the neighbor
                parent_map[(neighbor_x, neighbor_y)] = (current_x, current_y)
                
                # Mark the neighbor as visited by adding it to closed_list
                closed_list.append((neighbor_x, neighbor_y))
                
                # Add the neighbor to all_nodes for plotting
    ...
    # Plot the optimal path with a line
    if path:
        plot_optimal_path(path, ax)
